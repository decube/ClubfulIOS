    ...
    func setLayout(_ ctrl: MessageViewController, element: [String: AnyObject], isNew: Bool = false) -> CGFloat{
        ...
        DispatchQueue.global().async {
            ...
            DispatchQueue.main.async {
                ...
                layer1.fillColor = UIColor(hex: 0xcad9f1).cgColor
                layer2.fillColor = UIColor(hex: 0xcad9f1).cgColor
                layer3.fillColor = UIColor(hex: 0xcad9f1).cgColor
                ...
                layer2.path = UIBezierPath(roundedRect: CGRect(x: 0, y: 0, width: self.messageView.frame.width-10, height: self.messageView.frame.height), byRoundingCorners: [UIRectCorner.allCorners], cornerRadii: CGSize(width: 10, height: 10)).cgPath
                layer3.path = UIBezierPath(roundedRect: CGRect(x: self.messageView.frame.width-20, y: self.messageView.frame.height-20, width: 20, height: 20), byRoundingCorners: [UIRectCorner.bottomRight], cornerRadii: CGSize(width: 20, height: 20)).cgPath
