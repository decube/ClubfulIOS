...
class MypageInsViewController: UIViewController {
                ...
                if let list = dic["interestList"] as? [[String: AnyObject]]{
                    if list.count == 0{
                        ...
                        for data in list{
                            ...
                            let courtStar = Storage.getStorage("courtStar_\(court.seq)")
                            if courtStar == nil{
                                Storage.setStorage("courtStar_\(court.seq)", value: true as AnyObject)
                            }
    ...
    func layout(){
        ...
        if Storage.isRealmUser(){
            rm()
            if MypageInsViewController.isReload{
                MypageInsViewController.isReload = false
...
extension MypageInsViewController{
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "mypage_courtDetail"{
            let path = self.collectionView.indexPath(for: sender as! MyCourtInsCell)
...
extension MypageInsViewController: UICollectionViewDelegate{
    
}

extension MypageInsViewController: UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "MyCourtInsCell", for: indexPath) as! MyCourtInsCell
